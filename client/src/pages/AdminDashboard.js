...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch statistics
      const statsResponse = await axios.get('http://localhost:3000/api/admin/thong-ke');
      ...
      const tinDangResponse = await axios.get('http://localhost:3000/api/staff/tin-dang', {
        params: { 
          page: currentPage, 
          limit: 10, 
  ...
  const handleUserAction = async (userId, action, newValue = null) => {
    try {
      ...
      if (action === 'toggle_status') {
        endpoint = `http://localhost:3000/api/admin/users/${userId}/toggle-status`;
      } else if (action === 'change_role') {
        endpoint = `http://localhost:3000/api/admin/users/${userId}/role`;
                  ...
                  <div key={tinDang._id} className="post-card">
                    <div className="post-image">
                      {tinDang.images && tinDang.images.length > 0 ? (
                        <img 
                          src={`http://localhost:3000${tinDang.images[0]}`}
                          alt={tinDang.tieu_de}
                        />
                      ) : (
