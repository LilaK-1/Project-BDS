...
import './CreatePost.css'; // Sử dụng chung CSS với CreatePost
    ...
    const fetchPost = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          // Fetch images
          try {
            const imageResponse = await axios.get(`http://localhost:3000/api/images/tindang/${id}`, {
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
              }
            });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:3000/api/tindang/${id}`,
        formDataToSend,
        {
          headers: {
                ...
                <div className="image-grid">
                  {existingImages.map(image => (
                    <div key={image._id} className="image-item">
                      <img 
                        src={`http://localhost:3000${image.duong_dan_anh}`} 
                        alt="Existing" 
                      />
                      <button
                        type="button"
