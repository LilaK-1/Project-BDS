...
const TinDangDetail = () => {
  ...
  useEffect(() => {
    const fetchTinDang = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/tindang/${id}`);
  ...
  const handleApproval = async (action) => {
    try {
      const response = await axios.put(
        `http://localhost:3000/api/staff/tin-dang/${id}/duyet`,
        {
          trang_thai: action,
                  ...
                  <div className="seller-profile">
                    <div className="seller-avatar">
                      {tinDang.nguoi_dung_id.avatar ? (
                        <img 
                          src={`http://localhost:3000${tinDang.nguoi_dung_id.avatar}`}
                          alt="Avatar"
